...
//This Util Class will make sure the requests hitting our server has a JWT in the Authorization header
...
public class JwtTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        //Update the authentication context with the Employee details (ID, username, title).
    ...
    /* this method extracts the Employee details from the JWT token (including their authorities),
    creates an Authentication object with these details,
    and sets this Authentication object in the security context.
    This lets Spring determine who the Employee is and what privileges they have */
    private void setAuthenticationContext(String token, HttpServletRequest request) {
        Employee employee = (Employee) getEmployeeDetails(token);

        List<GrantedAuthority> authorities = List.of(new SimpleGrantedAuthority(employee.getTitle()));
        ...
        //After all the configurations above, we tell Spring who the employee is and what their title is
        //This information is stored in the authentication object we created above
        SecurityContextHolder.getContext().setAuthentication(authentication);
    }

    //This is used to extract the employeeId, username, and title from the JWT in the method above
    private UserDetails getEmployeeDetails(String token) {
        Employee employeeDetails = new Employee();

        //use the extractor methods we wrote in JwtTokenUtil to get the userId and username
        employeeDetails.setEmployeeid(jwtUtil.extractEmployeeid(token));
        employeeDetails.setUsername(jwtUtil.extractUsername(token));
        employeeDetails.setTitle(jwtUtil.extractTitle(token));

        System.out.println("employee: " + employeeDetails);
        System.out.println("title: " + employeeDetails.getTitle());
