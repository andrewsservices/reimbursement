...
//JWT (JSON Web Token) is a modern/stateless way to store a Employee's information and confirm their identity
//We will set up our app so that the Employee cannot do anything besides login/register
...
public class JwtTokenUtil {
    ...
    //we need the following 2 methods to get the username and title from the JWT (stored in the claims)
    ...
    public String extractTitle(String token) {
        Jws<Claims> claimsJws = Jwts.parserBuilder().setSigningKey(SECRET_KEY).build().parseClaimsJws(token);
        return claimsJws.getBody().get("title", String.class);
    }
